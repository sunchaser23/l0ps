...
result = subprocess.run(["poetry", "run", "python", "blocks.py"], capture_output=True, text=True)
if result.returncode == 1:
    logger.error("Running blocks failed")
    message = urllib.parse.quote(f"Project: {config['projectname']}: Running blocks failed")
